...
void UpdateChecker::helpDoc() {
    #ifdef Q_OS_MACOS
        const auto pdfPath = QDir::cleanPath(QCoreApplication::applicationDirPath() + "/../Resources/HelpDoc_v2_1_0.pdf");
    #else
        const auto pdfPath = QDir::cleanPath(QCoreApplication::applicationDirPath() + "/resources/HelpDoc_v2_1_0.pdf");
    #endif

    APP::messageCenter()->sendDebug(QString("HelpDoc path: %1").arg(pdfPath));

    if (!QFile::exists(pdfPath)) {
        APP::messageCenter()->sendWarning(QString("No such file: %1").arg(pdfPath));
        return;
    }
