...
bool DataBase::init() {
    ...
    APP::messageCenter()->sendDebug(QString("Database Dir: %1").arg(this->dataBaseDir));
...
Download DataBase::read(const qint64 id) const {
    ...
    if (q.next()) {
        ...
        const auto dst  = intToDownloadStatus(q.value("downloadStatus").toInt());
        ...
        return {
            id, url, tt, ac, vc, ndc, ba, bv, sp, suf, met, subt, prog, dst, db, tb, dsp, eta
        };
    }
...
QList<Download> DataBase::readAll() const {
    ...
    while (q.next()) {
        ...
        list.append(
            { idd, url, tt, ac, vc, ndc, ba, bv, sp, suf, met, subt, prog, dst, db, tb, dsp, eta}
        );
    }
