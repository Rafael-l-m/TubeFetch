...
void DbClient::addYoutubeAction(const YoutubeActions& download, std::function<void(QJsonObject)> cb) {
    this->sendRequest(
        Operation::AddYoutubeAction,
        Specification::Unknown,
        download.toJson(),
        std::move(cb)
    );
...
void DbClient::removeUselessActions(const qint64 timeLeft, std::function<void(QJsonObject)> cb) {
    QJsonObject params;

    params[JSON_FORMAT::TIME_LEFT] = timeLeft;

    this->sendRequest(
        Operation::DeleteUselessActions,
        Specification::Unknown,
        params,
        std::move(cb)
    );
...
void DbClient::getWeights(const qint64 timeLeft, std::function<void(QJsonObject)> cb) {
    QJsonObject params;
...
void DbClient::flushPending() {
    while (!this->pending.empty()) {
        ...
        this->sendRequest(op, spec, params, std::move(cb));
    }
}
    ...
    std::function<void(QJsonObject)> cb)
{
    if (this->socket->state() != QAbstractSocket::ConnectedState) {
        this->pending.enqueue({op, spec, params, std::move(cb)});
        return;
    }
