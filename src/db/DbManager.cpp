...
DbManager::DbManager(QObject* parent) : DataBase(parent) { init(); }

QJsonObject DbManager::isOpenDB() const {
    QJsonObject obj;
    obj[JSON_FORMAT::STATUS] = isOpen() ? JSON_FORMAT::STATUS_ACCEPTED : JSON_FORMAT::STATUS_REFUSED;
    return obj;
}
...
QJsonObject DbManager::updateDownloadStatus(const QJsonObject& obj) const {
    ...
    if (!DataBase::ifExists(internalId)) {
        ...
        const auto newDownloadStatus = intToDownloadStatus(obj[JSON_FORMAT::DOWNLOAD_STATUS].toInt());
