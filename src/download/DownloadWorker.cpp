...
DownloadWorker::DownloadWorker(
    ...
    QObject* parent) : QObject(parent), m_task(dt), m_process(process), retries(retries),
    fragmentRetries(fragmentRetries), concurrentRetries(concurrentRetries) {}

QProcess* DownloadWorker::getProcess() const { return this->m_process; }

qint64 DownloadWorker::getInternalId() const {
    return this->m_task ? this->m_task->getInternalId() : 0;
}
...
void DownloadWorker::start() {
    ...
    APP::messageCenter()->sendDebug(
        QString("DownloadWorker start with internalId: %1")
            .arg(this->getInternalId())
    );

    APP::messageCenter()->sendDebug(
        QString("Delay: %1; Rate: %2")
            .arg(randomDelayMs)
            .arg(randomKString)
    );
    ...
    QTimer::singleShot(randomDelayMs, this, [this, randomKString](){
        if (!this->m_process || !this->m_task) {
            ...
            APP::messageCenter()->sendError(
                "Process is null || DownloadTask is null || DownloadTask->Download is null when downloading"
            );
        ...
        arguments << "--ffmpeg-location" << ffmpegPath
                  ...
                  << "--fragment-retries" << QString::number(this->fragmentRetries)
                  << "--concurrent-fragments" << QString::number(this->concurrentRetries);
        ...
        connect(this->m_process, &QProcess::readyReadStandardOutput, this, [this](){
            ...
            APP::messageCenter()->sendInfo(
                QString("Standard output: %1")
                    .arg(text)
            );
        ...
        connect(this->m_process, &QProcess::readyReadStandardError, this, [this](){
            ...
            emit errorOccurred(this->getInternalId(), error);
        });

        connect(this->m_process, &QProcess::errorOccurred, this, [this](const QProcess::ProcessError) {
            const auto error = this->m_process->errorString();

            APP::messageCenter()->sendWarning(
                QString("Error occurred: %1")
                    .arg(error)
            );
...
void DownloadWorker::stop() {
    ...
    if (!this->m_process->waitForFinished(500)) {
        this->m_process->kill();
