...
void URLInfoManager::isValidUrl(const QString& url) {
    ...
    APP::messageCenter()->sendDebug(QString("Url '%1' is %2").arg(url_trimmed).arg(ifMatch ? "valid" : "invalid"));
}
    ...
    const QString& node_path)
{
    ...
    connect(this->process, &QProcess::errorOccurred, this, [this]() {
        ...
        APP::messageCenter()->sendError(QString("Error extracting information from url: %1").arg(err_str));
    });
...
void URLInfoManager::analyzeUrlInfo(const QJsonObject& obj) {
    ...
    for (const auto& value : formats) {
        ...
        const auto flag = re.match(str).hasMatch();
        ...
        if (str.contains("sb")) {
            otherFormats.append(str);
            continue;
        }

        if (videoYoutubeCodec.compare("images", Qt::CaseInsensitive) == 0) {
            otherFormats.append(str);
            continue;
        }
        ...
        if (videoYoutubeCodec.compare("audio only", Qt::CaseInsensitive) == 0 ||
            videoYoutubeCodec.compare("none", Qt::CaseInsensitive) == 0)
        {
            const auto audioCodec = MediaCodecFilters::parseAudioCodec(audioYoutubeCodec).trimmed().toLower();
            const auto finalFormat = QString("%1 (%2)").arg(str).arg(audioCodec);
        ...
        if (MediaCodecFilters::NON_DASH_FORMAT_IDS.contains(num)) {
            nonDashFormats.append(num);
            continue;
        }
        ...
        if (!videoYoutubeCodec.isEmpty() &&
            videoYoutubeCodec.compare("none", Qt::CaseInsensitive) != 0 &&
            videoYoutubeCodec.compare("audio only", Qt::CaseInsensitive) != 0 &&
            videoYoutubeCodec.compare("images", Qt::CaseInsensitive) != 0)
        {
            const auto videoCodec = MediaCodecFilters::parseVideoCodec(videoYoutubeCodec).trimmed().toLower();

            const auto match = re.match(str);

            QString displayId = str;

            if (match.hasMatch()) { displayId = QString("%1-sr").arg(match.captured(1)); }

            const auto finalFormat =QString("%1 (%2)").arg(displayId).arg(videoCodec);
...
void URLInfoManager::handleFinished(const int exitCode, const QProcess::ExitStatus& status) {
    ...
    APP::messageCenter()->sendDebug(QString("Exit Code: %1").arg(exitCode));
