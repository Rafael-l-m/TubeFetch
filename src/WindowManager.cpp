...
QQuickWindow* WindowManager::createWindow(
    ...
    const QVariantMap& params)
{
    const QUrl url(QStringLiteral("qrc:/qt/qml/YoutubeMediaDownloader/qml/window/%1").arg(fileName));

    QQmlComponent component(this->m_engine, url);

    if (component.status() == QQmlComponent::Error) {
        APP::messageCenter()->sendError(
            QString("Failed to load window: %1")
                .arg(fileName)
        );
    ...
    if (!obj) {
        APP::messageCenter()->sendError(
            QString("Failed to create window: %1")
                .arg(fileName)
        );
