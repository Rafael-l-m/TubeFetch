...
void LanguageManager::setLanguage(const QString& newLanguage) {
    ...
    #else
        const auto translationPath = QDir::cleanPath(QCoreApplication::applicationDirPath() + "/translations");
    #endif
    ...
    if (this->m_translator.load(fp)) {
        ...
        SYS_INFO::CONFIG::writeConfig( { {"languages", _newLanguage} } );
        ...
        APP::messageCenter()->sendError(
            QString("New language load failed: %1")
                .arg(_fileName)
        );
    }
}
